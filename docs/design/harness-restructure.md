...
- 狀態：草稿（待審）
- 日期：2026-09-17（2026-09-21 更新：PR #82 已合併，§2.1、§2.2、§5 隨之修訂；同日依 #86 review 修訂 §0、§1.3、§2.4、§2.6、§3、§4、§5、§6——發布契約先行、可攜核心＋轉接層、checker 不併）
...
| 規則 9（SDK 檔不可修改）、規則 14（VFS 上限；文件與腳本曾各抄一份 200、實為 500，已由 #85／#88 修正，見 §7）、禁用詞交付前 grep、憑證不得出現在指令列 | Guards | 這四條都能由程式直接判定對錯，寫成文字只是請求 |
...
| SKILL.md Phase 4.4 發布、Phase 5 的問題回報 submit | Workflows | 這兩者會影響外部系統，不該由模型自行決定何時執行（這是語意分類；實體落點是腳本的確認閘，見 §3.2） |
...
**(b) `scripts/check_update.py` 會把每一份已登記的安裝 `git fetch` → `git reset --hard` 到 main**（`:292–313`，3 小時節流）。若在本 repo 的 main 直接重組，merge 後數小時內所有既有安裝的根目錄 `SKILL.md` 消失；`resources/hooks/` 範本裡寫死的 `scripts/check_update.py` 路徑也一併失效，連自我修復的機會都沒有。
...
│   ├── aigo-migrate/               ← 遷入流程，自 builder 的 Phase 1.25／2.0 拆出
│   └── aigo-template-transfer/     ← transfer 的 S0～S9
...
| Rules | `skills/aigo-builder/references/rules/*.md` | ① skill 觸發時按需讀（所有客戶端）② plugin 的 SessionStart hook 注入一份精簡索引（30 行以內，只列硬規則與檔名）③ scaffold 寫進 app 專案的 `.claude/rules/`（帶 `paths`）與 `AGENTS.md`，附版本戳 |
| Agents | `agents/*.md` | plugin（Claude Code）；其他客戶端退回 skill 內的文字指示「可派 subagent 時，交給獨立 context 執行」 |
| Workflows | **不獨立成一類**——見 §3.2 | — |
...
| 共用程式以 `core/` 為正本、同步副本進各 skill 的 `scripts/_core/`，CI 守一致 | 不依賴安裝時的網路與套件來源；安裝後離線可用 | repo 內有重複檔案。替代方案（`scripts/pyproject.toml` 以 git 依賴安裝 `core`）列入 §6 |
...
| Workflows 不獨立成一類；「只能由人觸發」由 **腳本內的確認閘** 保證 | `disable-model-invocation` 是 Claude Code 專屬欄位（§3.3），換客戶端保護就消失。transfer 已經示範了可攜的做法——鐵律 2 的人工閘由腳本的互動確認擋住（§2.2）。publish、問題回報 submit、模板送審沿用同一招；plugin 可另附薄薄一層 slash command 當入口 | 確認閘要逐支腳本實作並測試 |
| Rules 的正本留在 skill 目錄內，常駐載入交給轉接層 | 正本只有一份、跟著 skill 走、由更新機制同步；落地到使用者專案的副本只是快取，靠版本戳提醒重產 | 落地副本會過期——scaffold 與 SessionStart hook 都要比對版本戳 |
...
| plugin 的 slash command（publish 等的薄入口，選配） | 同 skill 格式 | 另加 `disable-model-invocation: true`（見下方警告）；真正的把關在腳本的確認閘（§3.2） |
...
- `disable-model-invocation` 是 **Claude Code 專屬能力**；換到不支援的客戶端時，那層保護會消失。所以「只能由人觸發」不押在它身上，而是由腳本的確認閘把關（§3.2），slash command 只是入口
...
**仍然建議做的**（在 checker 自己的 repo）：依 §1.2 (a)，把這條規則從文字換成程式——CI 掃 `references/aigo-platform.md` 以外的所有檔案，出現產品名稱就失敗。§2.5 的 stack 檔降級、8 個維度改派唯讀 subagent 平行稽核，也都在該 repo 內進行。

**何時重新考慮併入**：checker 開始需要與 builder 共用程式（例如要呼叫平台 API 取證），屆時再評估。
...
| 有外部副作用的流程（publish 等） | tool 本身帶確認閘；可另附 prompt 當入口 | 有支援的客戶端會把 prompt 顯示成 slash command |
...
1. **在本 repo、現有結構內先做 guards**：`sdk_files`（規則 9）、`vfs_limits`（規則 14）、`forbidden_terms`（CONTEXT.md 禁用詞）寫成純函式，接進 `aigo_sync.py`／`aigo_publish.py`，比照現有的 `egress_preflight`。每支先寫測試再寫實作；SKILL.md 對應條文改為指向該 guard。這一步不搬任何檔案，既有安裝透過自我更新直接受益
...
3. **定發布契約、開新 repo**：依 §3.1 建骨架（`skills/`、`core/`、`.claude-plugin/`、`tests/`）；把步驟 2 的安裝 smoke test 搬過去並加上 plugin 一條路。此步同時查清 §6 的客戶端支援矩陣
4. **搬 builder**：`references/`、`resources/`、`scripts/` 搬進 `skills/aigo-builder/`，先原樣搬、不改內容；再抽 rules——`references/dev-rules.md` 全份、`references/environment.md` 的租戶網址與憑證規則、SKILL.md Phase 3 的規則 1–17 速查表，拆成 `references/rules/` 下五個檔；`check_docs.py` 的引用檢查同步改指新位置。遷入流程拆成 `skills/aigo-migrate/`
...
6. **轉接層**：`agents/`（app-inventory、report-verifier）、`hooks/hooks.json`（SessionStart 更新檢查＋精簡硬規則索引、PreToolUse guards）、`.claude-plugin/plugin.json`；scaffold 增加「把 rules 落地到 app 專案的 `.claude/rules/` 與 `AGENTS.md`，附版本戳」；publish、問題回報 submit、模板送審三支腳本補確認閘
7. **舊 repo 的過渡**：本 repo 維持現有結構、照常收修正，直到新 repo 穩定。之後在本 repo 發最後一版——`check_update.py` 改為「告知新位置與遷移指令」，**不**把既有安裝 reset 成新結構。transfer repo 同樣處理
...
- [ ] 共用程式的分享機制：`core/` 同步副本＋CI 一致性檢查（本文件的預設），或 `scripts/pyproject.toml` 以 git 依賴安裝 `core`（省掉重複檔案，但首次執行需要網路、要處理版本釘選）
- [ ] 版本與發布節奏：新 repo 用單一 `VERSION` 還是各 skill 各自一個？builder 改版頻繁（單日可達數版），transfer 的使用者是否要跟著被同步
