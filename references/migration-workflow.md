...
> **不論單系統或多系統，每個要遷入的系統都必須先過 §2.1 的產品線判斷**——
> 它的結果不可逆（見 §2.1 的警示）。
...
2. **逐系統做產品線判斷**（★ 不可省——結果不可逆）
   - 對清單上**每一個**系統跑一次 §2.1 的決策樹，記下產品線與模式
...
5. **產出：遷入全景表**
   - 格式：`| 外部系統 | 產品線（Custom / Hosted） | 模式（internal / external / visibility） | 對應 AI GO App | app_domain | 遷入順序 | 語意重疊的表 |`
...
> 依序執行 §2.1 → §2.5。§2.1 判走 Hosted App 的系統只需再做 §2.2（解構盤點）
> 與資料側的 §2.4／§2.5（資料落點見 `hosted-apps.md` §7.1），
> 開發與部署改走 `references/hosted-apps.md`，不進本 skill 的 Phase 2–4。

### 2.1 產品線與模式判斷（★ 最先做，結果不可逆）

用兩個問題把系統定位到四象限。**先問使用者是誰，再問技術形狀**——
...
| 答案 | 走向 |
|------|------|
| 願意在平台內重構（前端重寫進 Builder、後端改寫成 Server Action、資料改用平台承載） | **Custom App** |
| 整套原始碼原樣搬（自選框架、常駐進程、WebSocket、自訂網域、無法/不願重構） | **Hosted App** |
...
- 逐表對照（先跑 Phase 1.5 第 3 點的雙邊盤點）：
  - 與 ERP／SaaS 功能連動的資料 → SaaS 表原生欄位；無原生對應的欄位 →
