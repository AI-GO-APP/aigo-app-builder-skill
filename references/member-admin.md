...
`access_role_ids` 與租戶資料存取規則才是授權來源，app 名單上的 `can_login`／`is_admin` 這類欄位是
§3.6 說的「要被平台取代的東西」，遷入後只能當業務屬性，不能當第一道門的替代品。
「使用者資料」兩邊各一份是正常的：AI GO 只有帳號（信箱）與角色，app 的資料庫才有業務屬性；兩邊靠**同一個信箱**對上。

- **盤點**：遷入計畫要有一張「app 可登入名單 vs 租戶成員」對照（不在租戶／在租戶但沒角色／兩邊都齊三類）：
  `GET /api/v1/members`（§2，需 `hr.member_manage`）對 app 的名單比信箱與別名。
- **誰來邀**：把客戶的使用者邀進客戶的租戶，是**客戶（租戶管理員）的事**，不是遷入者自己批次跑——邀請會寄信給真人、
