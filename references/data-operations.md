...
> 2026-09-03 於 demo 租戶（`playground-super-admin`，`system.admin`）逐條實測；平台原始碼核對日期同。
...
- `system.admin` 在所有閘都直通（demo 的擁有者就是這樣）；一般員工帳號**未實測**，權限對照以上表為準。
...
## 4. 各模組 REST 的實測慣例（demo 2026-09-03）
