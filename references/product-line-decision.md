...
| 直連外部 DB、自帶 Postgres／Redis | ❌ **兩條線都禁**（規則 32）；網路層自 v1.15.2 起連得到，但那不是授權 | 資料一律進平台的表。唯一例外（平台工程師核准、逐項能力實測缺才能申請、**核准紀錄存在才生效**）見 **dev-rules.md 規則 32** |

標「兩條線都沒有／都禁」的是平台的**資料層能力邊界**，換 Hosted 解決不了——
要在需求階段改設計，不要對用戶說「換 Hosted 就好」。撞到交易／FK／唯一約束／列鎖／RLS 且改設計
不可行時，走規則 32 的例外申請（builder 只提申請，不自己開庫），不要自己繞。
