...
| **發布回 409 `EGRESS_NOT_READY`，但外部服務明明建好也授權了** | 第三種成因：服務**被停用**（`gaps[].kind = service_inactive` / code `egress_service_inactive`）。授權當下會擋停用中的服務，但**之後停用不會回收授權清單**，所以「已授權 ∧ 已停用」是可達狀態。去 Builder「外部服務」把它重新啟用，**不要再建一個同名的**（slug 唯一）。`scripts/aigo_publish.py` 的預檢會把這格單獨標成「服務存在但已停用」 → `platform-behaviors.md` §13.5 |
