...
| **走 `ctx.http.call` 的 action 提早被切（10 或 30 秒），manifest 明明寫 120000** | 這是 **egress 閘道**的 `timeout_ms`，設定在**那支外部服務**上：**預設 10000、硬上限 30000**（`PATCH` 60000／120000 回 422）。到期時平台砍掉**整支** action，`status: "timeout"`／`result: null`／`error: "Action 執行超時(<該服務的 timeout_ms>ms)"`——原文與 runner ceiling 那道**同形**，括號裡的毫秒數是閘道的值不是 manifest 的。**分辨法**：把對外呼叫換成純 `time.sleep(n)` 再試跑一次；同一支已發布 app 的 `sleep(100)` 回 `success`／`duration_ms 100002`＝ceiling 正常、牆在閘道（實測 `ctx.http.call` 那幾發全落在 30394～30405 ms）。想拿滿 30 秒請用戶在 Builder 把該服務的 `timeout_ms` 調到 30000（人工設定，AI 不代設；沒設就是 10 秒）。`ctx.http.call` 沒有 per-call timeout 參數（傳 `timeout=` 回 `TypeError`），串流也不繞過（砍的是單次呼叫的總時間不是 idle） → `custom-app-dev-guide.md` §25.4 |
...
| **發布回 409 `EGRESS_NOT_READY`，但外部服務明明建好也授權了** | 第三種成因：服務**被停用**——409 的 `code` 一律是 `EGRESS_NOT_READY`，要看 `gaps[].kind` 才知道，這格是 `service_inactive`（`message` 寫「存在，但目前是停用狀態」）。授權當下會擋停用中的服務，但**之後停用不會回收授權清單**，所以「已授權 ∧ 已停用」是可達狀態。請用戶到 Builder「外部服務」把它重新啟用（人工，AI 不代設），**不要再建一個同名的**（slug 唯一）。`scripts/aigo_publish.py` 的預檢會把這格單獨標成「服務存在但已停用」 → `platform-behaviors.md` §13.5 |
