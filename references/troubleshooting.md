...
| **只要 VFS 有 `actions/requirements.txt`，每支 action 就回 500 `An error occurred (403) when calling the HeadObject operation: Forbidden`** | 症狀（2026-09-09 單一租戶觀察）：第一發建 wheelhouse 會成功（wall 約 29 秒），之後每一發約 0.2 秒就 500；刪掉 `requirements.txt` 立刻恢復 200、放回又 500，退避重打 12 次／9 分鐘全數 500，`validate` 仍回 `valid: true`。**分辨法**：看有沒有 `HeadObject` 字樣、以及「有／無 `requirements.txt`」是不是乾淨互斥——是的話與 pin 寫錯無關（那是 422 `WHEELHOUSE_RESOLVE_FAILED` 並附 uv 的解析原文，見上兩列），也與冷啟慢無關。只在一個租戶看過、沒有對照組，**不要據此斷定是平台故障**；照 `issue-reporting.md` 把隔離實驗的原始回應收齊再回報 → `custom-app-dev-guide.md` §16.2 |
...
| **走 `ctx.http.call` 的 action 在 30 秒被切，manifest 明明寫 120000** | 這是 **egress 閘道**的 `timeout_ms`（硬上限 30000；`PATCH` 60000／120000 回 422），到期時平台砍掉**整支** action，`status: "timeout"`／`result: null`／`error: "Action 執行超時(30000ms)"`——原文與 runner ceiling 那道**一字不差**，訊息裡的 `30000ms` 是閘道的值不是 manifest 的。**分辨法**：把對外呼叫換成純 `time.sleep(n)` 再試跑一次；同一支已發布 app 的 `sleep(100)` 回 `success`／`duration_ms 100002`＝ceiling 正常、牆在閘道（實測 `ctx.http.call` 那幾發全落在 30394～30405 ms）。`ctx.http.call` 沒有 per-call timeout 參數（傳 `timeout=` 回 `TypeError`），串流也不繞過（砍的是總時間不是 idle） → `custom-app-dev-guide.md` §25.4 |
...
| **已發布 app 閒置一陣子後第一次呼叫 action 很慢／逾時，之後就正常** | runner scale-to-zero 冷啟動，不是 action 壞，**也不是開常駐的理由**——先問 owner 實際容忍秒數（dev-guide §28）；容忍不了且為付費租戶才 `PATCH /api/v1/builder/apps/{id}/runtime-settings {"always_on": true}`（`builder.publish`）或在 Builder App 設定「執行模式」切常駐，並寫下理由與退場條件；免費 403 `ALWAYS_ON_REQUIRES_PAID_PLAN`、未發布 422、綁通訊渠道的 app 本來就常駐（`locked_reason: messaging_trigger`）→ `custom-app-dev-guide.md` §28 |
