...
33. **每支要上正式的 app 都要有一個「UAT 結論」**（★ 強制，2026-09-21 立；做法見 `uat-environment.md`）
    - 計畫階段寫一句：`UAT＝有；拓撲 X` 或 `UAT＝無；理由 Y；風險 Z`——判準是**有沒有正式資料會被測試污染、
      有沒有對外副作用**；兩者都沒有的 app 可以寫「無」。沒寫＝沒過閘（落點：需求盤點表 §四.2、Phase 5 里程碑、`hosted-apps.md` §3.4）
    - 有 UAT 時：鏡像該系統**實際拓撲**（Custom-only／Hosted＋入口／多服務各不同），不是一律「clone 一顆 Hosted」；
      所有適用資源用 `-uat` 命名（Custom App `url_name`、Hosted slug、外接庫專案、egress slug）
    - **資料庫、憑證、Open Proxy 引用、外部副作用目的地全部獨立**；UAT app 不得讀寫正式表或正式庫。
      **租戶只有一個**——UAT 入口掛 UAT 專用角色，只發給測試者，不沿用正式白名單、不為 UAT 邀整批人
    - 排程、常駐 worker、通知、webhook **預設停用**，完成資料隔離、權限、secret、403 對照、
      自檢與回滾驗證後才逐項啟用
