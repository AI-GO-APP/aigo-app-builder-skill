...
1.46.1 把文件上的上限改成平台實際值 500，但 `scripts/aigo_sync.py` 的 `MAX_FILE_COUNT`
還是 200——超過 200 檔的專案在 `read_local_files` 就被本地腳本擋下，根本送不到平台。
改為 500，與 `infra/builder/compile.go` 的 `MaxFileCount` 一致。

一併修正單檔大小的比法：原本比的是**字元數**，平台比的是 **bytes**。中日韓文字多的檔案
可能字元數未滿 100 萬、bytes 已超過，本地放行後被平台**靜默跳過**（1.46.1 補的那條警告）。
改為以 UTF-8 bytes 判定，超限在本地就報錯。
