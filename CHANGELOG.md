...
修掉舊機制的結構性盲區：節流狀態全域共用且只看時間，本機第二份（含以後）
安裝的檢查會被第一份**無限期靜默**——過時副本永遠不會提示更新，
破壞性變更的警告也一併失效。

- **節流改管「網路抓取與重複提示」，版本比對每次都做**：遠端 VERSION 抓一次
  快取 3 小時（`remote_cache`），任何安裝在節流窗內都能用快取發現自己落後；
  同一組版本差 3 小時內只提示一次，且抑制計時只在實際提示時刷新
- **多安裝註冊表**：每份安裝執行檢查時登記自身路徑，偵測到新版時列出本機
  其他落後的已註冊安裝；新增 `--apply-all` 一次更新所有落後的 git 安裝
  （複製式安裝維持只列指令不代動；路徑消失自動剔除）
- 舊版狀態鍵（頂層 `last_check` 等）不讀不寫不刪，與尚未更新的舊副本共存
- `VERSION` 與狀態檔改用 `utf-8-sig` 讀取，容忍 Windows 編輯器寫入的 BOM
